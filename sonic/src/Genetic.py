...
class Gene:
    ...
    def generate_parent(self):
        ...
        for i in np.arange(self.len_gene):
            ...
            pos1, pos2, pos3, pos4 = random.sample(self.pos_list, 4)
            mov_ = random.sample(self.mov, 4)
            temp[pos1] = mov_[0]
            temp[pos2] = mov_[1]
            temp[pos3] = mov_[2]
            temp[pos4] = mov_[3]
...
def selection_parent(population, target=None):
    ...
    # TODO: dividir el Fitness entre la posición Objetivo
    # winer = population.Genes
    # for i in range(len(population)):
    #
    # se guardan dos padres
    for part in range(parti_len):
        if len(participante) > 1:
            ...
            if part1.Fitness // 9546 > part2.Fitness // 9546:
                winer.append(part1)
...
def crossover(parents):
    ...
    new_polutation = [None]*10

    for i in range(10 - pressure):  # longitud de la poblacion - los padres
        new_polutation[i] = createChild(parents[0].Genes, parents[1].Genes)

    new_polutation[8] = parents[0]
    new_polutation[9] = parents[1]
